"""
QuotaVerace Bot – Modulo Telegram per segnali Poisson
Dati: Serie A 2025/26 | Kelly Criterion | Quote reali via API
"""
...
# === Health check server per UptimeRobot ===
class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.send_response(200)
        self.send_header("Content-type", "text/plain")
        self.end_headers()
        self.wfile.write(b"OK - QuotaVerace Bot is running")
...
def get_odds_data():
    """Carica quote: reali se API key presente, altrimenti statiche."""
...
async def cmd_segnale(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        best_over = max((o for o in odds_data if o.get("evento", "").lower() == event_name.lower() and o.get("esito", "").lower() == "over 2.5"), key=lambda x: x.get("quota_decimale", 0), default=None)
        if best_over:
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    squadre = "\n".join(f"• {s}" for s in sorted(SERIE_A_2025_26.keys()))
    text = (
        "📋 *Comandi QuotaVerace*\n\n"
        ...
        "🏟 *Squadre disponibili (Serie A 2025/26):*\n"
        f"{squadre}\n\n"
...
def main() -> None:
    ...
    logger.info("QuotaVerace Bot avviato. Health server su porta 8080.")
